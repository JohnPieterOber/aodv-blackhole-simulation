...
Ipv4Address fakeAddress = Ipv4Address("0.0.0.0");
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::RoutingProtocol")
    ...
    .AddAttribute ("IsMalicious", "Is the node malicious",
                   ...
                   MakeBooleanChecker ())
  .AddAttribute ("IsDetectBlackhole", "Can the node detect black holes",
                     BooleanValue (false),
                     MakeBooleanAccessor (&RoutingProtocol::SetDetectBlackholeEnable,
                                          &RoutingProtocol::GetDetectBlackholeEnable),
                   MakeBooleanChecker ())
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  if (IsDetectBlackhole && fakeAddress.IsEqual(Ipv4Address("0.0.0.0")))
  { 
   printf("Sending fake RREQ to detect Blackhole\n");
   fakeAddress = Ipv4Address("10.2.2.99");
   SetFakeAddress( fakeAddress);
   SendRequest(fakeAddress);
   }
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  //check RREPs for fake address
  if (IsDetectBlackhole && dst.IsEqual(GetFakeAddress()))
  {
  printf("Black hole detected!\n");
  }
