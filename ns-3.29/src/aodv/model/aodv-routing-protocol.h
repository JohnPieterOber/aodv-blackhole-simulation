...
public:
  ...
  void SetDetectBlackholeEnable (bool f) { IsDetectBlackhole = f; }                         
  bool GetDetectBlackholeEnable () const { return IsDetectBlackhole; }      
  void SetFakeAddress(Ipv4Address f) { FakeAddress = f;}
  Ipv4Address GetFakeAddress() const {return FakeAddress;}  
...
private:
  ...
  // gives the node the ability to detect black holes
  bool IsDetectBlackhole;  
  // the fake address used by the detector node
  Ipv4Address FakeAddress;
